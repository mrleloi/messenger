...
use App\Broadcasting\NewMessageBroadcast;
use App\Contracts\BroadcastDriver;
use App\Contracts\MessengerProvider;
use App\Http\Request\BaseMessageRequest;
...
abstract class NewSystemMessageAction extends BaseMessengerAction
{
    /**
     * @var BroadcastDriver
     */
    ...
    private bool $pendingStatus = true;
     ...
     * @return void
     */
    protected function fireEvents(Thread $thread,
                                  MessengerProvider $provider,
                                  string $body,
                                  int $type): void
    {
        if ($this->shouldFireEvents()) {
            $this->dispatcher->dispatch(new NewSystemMessageEvent($thread, $provider, $body, $type));
        }
    }
     ...
     * @return void
     */
    protected function storeMessage(): self
    {
        $this->setMessage(
    ...
    protected function storeTempMessage(): self
    {
        $msg = new Message([
            'type' => $this->messageType,
            'owner_id' => $this->messageOwner->getKey(),
