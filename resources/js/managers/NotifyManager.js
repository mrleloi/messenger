...
window.NotifyManager = (function () {
    ...
    broadcaster = {
        Echo : function(reconnected){
            ...
            /*if(Messenger.common().SOCKET_REDIS){
                opt.socket.Echo.connector.socket.on('connect', () => {
                ...
                opt.socket.Echo.connector.socket.on('disconnect', () => {
                    broadcaster.clearPusherLocalStorage();
    ...
    templates = {
        ...
        pending_friend : function (friend) {
            return '<a onclick="return false;" href="#" class="list-group-item list-group-item-action p-2 text-dark bg-light">\n' +
                ...
                '                   <button title="Accept fr="Deny friend onclick="NotifyManager.pendingFriends(\''+friend.id+'\', \'accept\')" class="btn btn-sm btn-success pt-1 pb-0 px-1"><i class="h5 far fa-check-circle"></i></button>' +
                '                   <button titlerequest" onclick="NotifyManager.pendingFriends(\''+friend.id+'\', \'deny\')" class="btn btn-sm btn-danger mx-1 pt-1 pb-0 px-1"><i class="h5 fas fa-ban"></i></button>' +
                '               </span>' +
                '            </div>\n' +
                '        </div>\n' +
                '    </div>\n' +
